...
Call:
```javascript
const token = await graph.unit(url, options);
```
...
Call:
```javascript
const token = await graph.list(url, options);
```
...
```
Call: Parallelism or linearly 
```javascript
const token = await graph.massive(urlPattern, values, options);
```
